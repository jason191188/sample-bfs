...
def format_path(end: int, start: int, path: list[int], directions: list[str], final_node: int, start_display: str = None, final_display: str = None) -> str:
    ...
    Args:
        ...
        final_display: 목적지 표시 문자열 ("4-1" 등), None이면 final_node 사용
    """
    # final_direction: 마지막 진행방향
    final_direction = directions[-1]

    start_str = start_display if start_display else str(start)
    final_str = final_display if final_display else str(final_node)
    result = f"{final_str}/{final_direction}~{end}!{start_str},{directions[0]}/"
